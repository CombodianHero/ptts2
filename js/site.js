...
function renderFooter() {
  document.getElementById("site-footer").innerHTML = `
    <div class="container footer-grid">
      ...
      <div><h4>Company</h4><ul><li><a href="#why-us">About</a></li><li><a href="#contact">Contact</a></li><li><a href="#reviews">Reviews</a></li><li><a href="#gallery">Gallery</a></li></ul></div>
...
/* Creates a booking record (status: PAYMENT_REQUIRED) and persists it. */
function createBooking(draft) {
  DB = loadDB();
  const bookingId = nextBookingId(DB);
  const booking = {
    bookingId,
    id: uid("bk"),
    name: draft.name || "",
    ...
    email: draft.email || "",
    pickup: draft.pickup || "Sasaram",
    destination: draft.destination || "",
    travelDate: draft.travelDate || "",
    returnDate: draft.returnDate || "",
    passengers: draft.passengers || "",
    vehicle: draft.vehicle || "",
    serviceType: draft.serviceType || draft.tripType || "Local",
    message: draft.notes || draft.message || "",
    ...
    receipt: null,
    invoiceId: null,
    charges: { vehicle: 0, driver: 0, other: 0 },
  };
  DB.bookings.unshift(booking);
  saveDB(DB);
  sendTransactionalEmail("booking_received", booking);
  return booking;
}
...
function showBookingConfirmation(booking) {
  ...
  box.innerHTML = `
    ...
    <div class="success-block">
      ...
      <p style="margin-top:8px;">Your enquiry has been saved as <b>${booking.bookingId}</b>. Our team will confirm your quote and share the payment link shortly.</p>
      <div class="modal-actions" style="justify-content:center;margin-top:22px;">
        ...
        <a class="btn btn-primary" target="_blank" rel="noopener" href="${waLink(DB.settings.whatsapp, waBookingMessage(DB.settings, booking))}">${icon("message", 15)} Share on WhatsApp</a>
      </div>
      <p style="font-size:11px;color:var(--text-500);margin-top:16px;">This is a demo booking flow using local browser storage. In production this would notify the business owner in real time.</p>
    </div>`;
}
